...
```bash
crucible run --tag run1 --profile                # 追蹤每次迭代的 token 用量
crucible postmortem --tag run1 --tokens           # 實驗結束後分析
```

顯示 prompt 各區段佔比、cache 命中率及每次迭代的耗時。詳見 [docs/PROFILING.md](docs/PROFILING.md)。
...
- [Token 分析](docs/PROFILING.zh-TW.md) — 追蹤 prompt 組成、cache 效率、每輪計時分解
- [FAQ](docs/FAQ.zh-TW.md) — 局部最優、單一指標、平行 agent、安全性、監控
- [Token 用量分析](docs/PROFILING.md) — token 用量、prompt 佔比、cache 效率分析
